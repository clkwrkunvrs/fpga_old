...
<xsl:param name="INSTANCE" select="'opb_mdm_0'"/>
...
<xsl:variable name="ports_left_cnt_" select="count(PORT[((@IS_INMHS = 'TRUE') and ((@DIR='I' or @DIR = 'IO') and not(@SIGNAME = '__NOC__') and not(@SIGNAME = '__DEF__') and (@IS_INMHS = 'TRUE')))])"/>	
<xsl:variable name="ports_rght_cnt_" select="count(PORT[((@IS_INMHS = 'TRUE') and ((@DIR='O') and not(@SIGNAME = '__NOC__') and not(@SIGNAME = '__DEF__')))])"/>	
	
<xsl:variable name="bifs_left_cnt_"  select="count(BUSINTERFACE[((@IS_INMHS = 'TRUE') and not(@BUSNAME = '__DEF__') and not(@BUSNAME = '__NOC__') and ((@IS_VALID = 'TRUE') or not(@IS_VALID)) and ((@TYPE = 'SLAVE')  or (@TYPE = 'TARGET')       or (@TYPE ='USER')))])"/>
<xsl:variable name="bifs_rght_cnt_"  select="count(BUSINTERFACE[((@IS_INMHS = 'TRUE') and not(@BUSNAME = '__DEF__') and not(@BUSNAME = '__NOC__') and ((@IS_VALID = 'TRUE') or not(@IS_VALID)) and ((@TYPE = 'MASTER') or (@TYPE = 'MASTER_SLAVE') or (@TYPE = 'INITIATOR') or (@TYPE ='MONITOR')))])"/>
...
<xsl:template name="Draw_Ports">
	...
	<xsl:for-each select="PORT[((@DIR='I'or @DIR='IO') and  (@IS_INMHS = 'TRUE'))]">
		<xsl:sort data-type="number" select="@IPD_INDEX" order="ascending"/>
	...
	<xsl:for-each select="PORT[((@DIR='O') and (@IS_INMHS = 'TRUE'))]">
		<xsl:sort data-type="number" select="@IPD_INDEX" order="ascending"/>
...
<xsl:template name="Draw_Bifs">
	...
	<xsl:for-each select="BUSINTERFACE[((@IS_INMHS = 'TRUE') and (not(@TYPE) or (@TYPE = 'SLAVE') or (@TYPE = 'TARGET') or (@TYPE = 'USER')))]">
	...
	<xsl:for-each select="BUSINTERFACE[((@IS_INMHS = 'TRUE') and ((@TYPE = 'MASTER') or (@TYPE = 'MASTER_SLAVE') or (@TYPE = 'INITIATOR') or (@TYPE = 'MONITOR')))]">
