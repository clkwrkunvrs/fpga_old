...
<xsl:include href="MdtTinySvgBLKD_StyleDefs.xsl"/>
...
<!--			
			...
			<xsl:message>mpmcBifRank <xsl:value-of select="$mpmcBifType_"/></xsl:message>
-->			
...
<!-- 
			<text class="p2pbus_label" 
				  x="{$bcArrow_X_ + $BLKD_BUS_ARROW_W + 6}"
				  y="{$bcArrow_Y_ + ceiling($mpmcBusHeight_ div 2) + 6}">
				  <xsl:value-of select="$busName_"/></text>	
-->			
...
-->			
		<xsl:call-template name="F_WriteText">
			<xsl:with-param name="iX" 		select="'12'"/>
			<xsl:with-param name="iY" 		select="'85'"/>
