...
-->	
	...
	<xsl:variable name="busColor_">
		<xsl:call-template name="BusStd2Color">
...
<!--						
		...
		<xsl:message>Proc Bif Rank <xsl:value-of select="$procBifType_"/></xsl:message>
-->
...
<!--	
	<xsl:message>Bif Rank <xsl:value-of select="$cmplxBifType_"/></xsl:message>
-->	
	<xsl:variable name="vert_line_x_"  select="($iLaneInSpace_X +  ceiling($BLKD_BIFC_W div 2))"/>
...
<xsl:template name="BCLaneSpace_PointToPoint">	
	...
	<xsl:variable name="busColor_lt_">
		<xsl:call-template name="BusStd2LightColor">
			<xsl:with-param name="iBusStd" select="$iBusStd"/>
		</xsl:call-template>	
	</xsl:variable>	
...
-->				
			...
			<xsl:choose>
				...
				<xsl:when test="@IS_PROCCONN and @BIF_Y">
					...
					<MULTICONN X="{$bcProc_X_}" Y="{$bcProc_Y_}" BUSNAME="{$procBusName_}" BUSSTD="{$iBusStd}" TYPE="{$procBifType_}" BIFSIDE="{$procBifSide_}"/>
				</xsl:when>
...
-->					
					...
					<MULTICONN X="{$bcMod_X_}" Y="{$bcMod_Y_}" BUSNAME="{$modBusName_}" BUSSTD="{$iBusStd}" TYPE="{$modBifType_}" BIFSIDE="{$modBifSide_}"/>
<!--					
...
<!--	
		<xsl:variable name="topRnk_" select="(exsl:node-set($multiConns_)/MULTICONN[(@Y = $busTop_)]/@TYPE)"/>
		<xsl:variable name="botRnk_" select="(exsl:node-set($multiConns_)/MULTICONN[(@Y = $busBot_)]/@TYPE)"/>
-->	
...
-->	
		...
		<xsl:variable name="leftRnk_"  select="(exsl:node-set($proc2procConn_heights_)/PR2PRCONN[(@X = $bcLeft_)]/@TYPE)"/>
		<xsl:variable name="rghtRnk_"  select="(exsl:node-set($proc2procConn_heights_)/PR2PRCONN[(@X = $bcRght_)]/@TYPE)"/>
...
-->					
	...
	<xsl:call-template name="Draw_SplitConnBus">
		...
		<xsl:with-param name="iBc_Rnk"  select="$splitBifType_"/>
...
-->	
	...
	<g id="{$space_name_}">
<!--
	<xsl:if test="$stackToWest_W_ = 0">
		<rect x="0" 
	  	  	  y="0"  
			  width= "{$space_W_ + $stackToWest_W_ + ceiling($stackToEast_W_ div 2)}"
		 	  height="{$space_H_}"
