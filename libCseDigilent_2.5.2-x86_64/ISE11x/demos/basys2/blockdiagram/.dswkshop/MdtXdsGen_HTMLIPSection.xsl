...
-->		
		...
		<TD COLSPAN="5" WIDTH="100%" ALIGN="CENTER" BGCOLOR="{$COL_XLNX}"><SPAN style="color:{$COL_WHITE}; font: bold 12px Verdana,Arial,Helvetica,sans-serif">Parameters</SPAN></TD>
			...
			<TD COLSPAN="5" WIDTH="100%" ALIGN="LEFT" BGCOLOR="{$COL_WHITE}">
				...
				<SPAN style="color:{$COL_MODSYSNW}; font: bold 9px Verdana,Arial,Helvetica,sans-serif">blue</SPAN>
				<SPAN style="color:{$COL_INFO}; font: bold 9px Verdana,Arial,Helvetica,sans-serif">
					...
					<xsl:variable name="name_bg_col_">
						<xsl:choose>
							...
							<xsl:when test="@CHANGEDBY='USER'">
								<xsl:value-of select="$COL_MODUSR"/>
							</xsl:when>		
								
							<xsl:when test="@CHANGEDBY='SYSTEM'">
								<xsl:value-of select="$COL_MODSYS"/>
							</xsl:when>		
...
-->	
	...
	<TABLE BGCOLOR="{$COL_BLACK}" WIDTH="{ceiling($G_MAIN_WIDTH div 2)}" COLS="7" CELLSPACING="1" CELLPADDING="1" BORDER="0">
		...
		<xsl:if test="PORT[(@IS_INMHS)]">
			...
			<TH COLSPAN="1" WIDTH="10%" ALIGN="CENTER" BGCOLOR="{$COL_GRAY}"><SPAN style="color:{$COL_BLACK}; font: bold 10px Verdana,Arial,Helvetica,sans-serif">[LSB:MSB]</SPAN></TH>h
			...
			<xsl:for-each select="PORT[(not(@SIGNAME = '__NOC__') and not(@SIGNAME = '__DEF__') and (@IS_INMHS))]">
		...
		<xsl:if test="BUSINTERFACE[(not(@BUSNAME = '__NOC__') and not(@IS_VALID = 'FALSE'))]">
			...
			<TH COLSPAN="1" WIDTH="25%" ALIGN="CENTER" BGCOLOR="{$COL_GRAY_DK}"><SPAN style="color:{$COL_BLACK}; font: bold 10px Verdana,Arial,Helvetica,sans-serif">Point 2 Point</SPAN></TH>
			...
			<xsl:for-each select="BUSINTERFACE[(not(@BUSNAME = '__NOC__') and not(@BUSNAME = '__DEF__'))]">
				...
				<xsl:variable name="p2pname_">
					<xsl:choose>
						<xsl:when test="(@BUSSTD and not((@BUSSTD='PLB') or (@BUSSTD='OPB')) or (@TYPE='TRANSPARENT'))">
							<xsl:value-of select="../../MODULE[not(@INSTANCE = $instance_) and BUSINTERFACE[(@BUSNAME = $busname_)]]/@INSTANCE"/>
						</xsl:when>
						<xsl:when test="not(@BUSSTD) or (@BUSSTD='OPB') or (@BUSSTD='PLB') or (@TYPE='TRANSPARENT')">NA</xsl:when>
				   </xsl:choose>
				</xsl:variable>					
				...
				<xsl:if test="$p2pname_ = 'NA'">
					<TD COLSPAN="1" WIDTH="25%" ALIGN="CENTER" BGCOLOR="{$COL_WHITE}"><SPAN style="color:{$COL_BLACK}; font: normal 12px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="$p2pname_"/></SPAN></TD>
				</xsl:if>
				<xsl:if test="not($p2pname_ = 'NA')">
				    <TD COLSPAN="1" WIDTH="25%" ALIGN="CENTER" BGCOLOR="{$COL_WHITE}"><A HREF="#ANCHOR_{$p2pname_}" style="text-decoration:none"><SPAN style="color:{$COL_XLNX}; font: italic 12px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="$p2pname_"/></SPAN></A></TD>
				</xsl:if>
			</xsl:for-each>
		</xsl:if>
		...
		<xsl:if test="@MODCLASS='BUS'">
			...
			<xsl:for-each select="../MODULE/BUSINTERFACE[(@BUSNAME = $instance_)]">
				...
				<TR></TR>
				<TD COLSPAN="5" WIDTH="50%" ALIGN="CENTER" BGCOLOR="{$COL_WHITE}"><A HREF="#ANCHOR_{$buscName_}" style="text-decoration:none"><SPAN style="color:{$COL_XLNX}; font: italic 12px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="$buscName_"/></SPAN></A></TD>
			...
			<TH COLSPAN="7" WIDTH="100%" ALIGN="CENTER" BGCOLOR="{$COL_XLNX}">
				...
				<xsl:sort data-type="number" select="@PRIORITY" order="ascending"/>
					...
					<xsl:for-each select="../../../MODULE[PORT[((@DIR='O') and (@SIGNAME = $intrsrc_))]]">
						...
						<TH COLSPAN="3" WIDTH="55%" ALIGN="CENTER" BGCOLOR="{$COL_WHITE}"><SPAN style="color:{$COL_BLACK}; font: normal 12px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="$intrsrc_"/></SPAN></TH>
						<TH COLSPAN="3" WIDTH="35%" ALIGN="CENTER" BGCOLOR="{$COL_WHITE}"><A HREF="#ANCHOR_{@INSTANCE}" style="text-decoration:none"><SPAN style="color:{$COL_XLNX}; font: normal 12px Verdana,Arial,Helvetica,sans-serif"><xsl:value-of select="@INSTANCE"/></SPAN></A></TH>
					</xsl:for-each>
				</xsl:for-each>
			</TH>
		</xsl:if>
